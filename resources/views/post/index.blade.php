      ...
      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
       ...
       <form action="{{ route('post.search') }}" method="POST" class="d-flex">
        ...
        <span class="input-group-btn">
            <button class="btn btn-outline-success" type="submit"><span class="fas fa-search" aria-hidden="true"></span>Buscar</button>
        </span>
      </form>
    </div>
  </div>
    ...
    <tbody>
        @if (count($posts) >= 1)
            ...
            <tr>
                ...
                <td>{{$post->title }}</td>
                <td>{{$post->author }}</td>
                <td> <a href="{{ route("post.edit",$post->id)}}" class="btn btn-sm btn-danger">Editar</a>
            ...
            @endforeach
        @else
            <tr> <td scope="row">{{"no encontro resultados"}}</td><td>
        @endif
    </tbody>
</table>
@endsection
